...
class MythAuditCommandTest extends TestCase
{
    ...
    public function myth_auditor_is_read_only_and_prioritizes_poor_legacy_content(): void
    {
        ...
        DB::table('mitos')->insert([
            ...
            [
                'titulo' => 'Mito Popular Auditor',
                'slug' => 'mito-popular-auditor',
                'mito' => '<p>Texto breve.</p>',
                'foto' => null,
                'publicar' => now(),
                'user_id' => $user->id,
        ...
        $exitCode = Artisan::call('mfa:myths:audit', ['--active' => true, '--limit' => 1000]);
        ...
        $this->assertStringContainsString('Mito Popular Auditor', $output);
        $this->assertStringContainsString('P1', $output);
        $this->assertLessThan(
            strpos($output, 'Mito Incompleto Auditor'),
            strpos($output, 'Mito Popular Auditor')
        );
