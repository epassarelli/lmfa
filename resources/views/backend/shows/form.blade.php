<div class="form-group">
  <label for="show">Nombre del Show</label>
...
<div class="form-group">
  ...
  @if (isset($show) && $show->foto)
    <div class="mt-2">
      <img src="{{ asset('storage/' . $show->foto) }}" alt="Foto de {{ $show->show }}" style="max-height: 80px;">
    </div>
  @endif
</div>
...
<div class="form-group">
  ...
  <input type="datetime-local" name="fecha" id="fecha" class="form-control" {{-- value="{{ old('fecha', isset($show) ? $show->fecha->format('Y-m-d\TH:i') : '') }}" required> --}} </div>
  ...
  <div class="form-group">
    ...
    <input type="datetime-local" name="publicar" id="publicar" class="form-control"
      value="{{ old('publicar', isset($show) ? $show->publicar->format('Y-m-d\TH:i') : '') }}" required>
  </div>
