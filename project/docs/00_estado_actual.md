...
- **Penas:** directorio evergreen en desarrollo sobre `penia_profiles`, separado del legado vacio. Cuenta con migraciones locales aplicadas, perfil verificado por responsable interno, API protegida, landing, ficha, sitemap, NAV, escenario demo local, CRUD administrativo, preview noindex y acciones explícitas de publicación/despublicación/archivado. El auditor read-only `mfa:penias:audit` reporta score/P1-P3/CSV. Faltan lote editorial real y gate de release; no se debe activar ni migrar contenido real aun.
