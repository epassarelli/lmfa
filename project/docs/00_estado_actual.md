...
- **Penas:** módulo evergreen cerrado para DEV sobre `penia_profiles`. El legado vacío `penias`, sus rutas, controlador, modelo y vista fueron retirados el 2026-09-03; no hay redirección porque no existían registros ni equivalencias. Incluye API, landing, ficha con acceso a la búsqueda canónica y descubrimiento de Peñas de la misma provincia, sitemap, NAV, CRUD, preview noindex, acciones editoriales, auditor read-only `mfa:penias:audit` y 10 fichas demo publicadas con 10 eventos futuros. Los tests cubren permisos, publicación, filtros/paginación, SEO y agenda visible. La activación en producción sigue requiriendo un lote editorial real con fuentes y derechos verificados.
