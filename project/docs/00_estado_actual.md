...
> Ultima actualizacion: 2026-09-03 (punto 7 en curso: gate completo de CI y smoke reproducible incorporados; exposición pública aún apagada)
...
| 7 | Ejecutar tests, staging, smoke y recién entonces fusionar | `in progress`: gate completo de CI incorporado; staging y smoke reales pendientes |
...
- **Confiabilidad de `dev`:** se eliminaron los conflictos de merge que habían quedado versionados en el workflow de CI, doce acciones Fortify/Jetstream y sus pruebas, factories y policy huérfanas, además de 72 artefactos distribuidos de CKEditor. Las pruebas de registro y recuperación de contraseña ahora cubren las rutas reales de Laravel UI. El workflow reúne las suites de Festivales, Festival Vivo, Artistas, Recetas, Mitos, Música, Peñas y Radios; agrega gates para los flags independientes, la preservación de datos legacy de Peñas, la carga idempotente del lote piloto real, los contratos/preflight de Content Refresh, `npm ci`, compilación Vite y la suite PHPUnit completa. El punto 6 quedó verde en GitHub Actions en el commit remoto `54a76ba` (run `33808875829`). El primer gate completo del punto 7 validó todos los controles específicos y detectó el remanente legacy de autenticación; su repetición está en curso. El workflow manual `Directory staging smoke` y `scripts/smoke-directories.sh` dejan reproducible el smoke con flags apagados y encendidos, pero todavía falta ejecutarlo contra un staging HTTPS real.
